...
SELECT
  ...
  DECODE(B.LIMIT_CD, 'CL001',  NVL(A.SYNC_AMT, 0) + NVL(B.SAVE_AMT,0), 'CL003', B.AMT) AMT
...
SELECT
  B.BUSINESS_NO, B.CARD_NO,
  DECODE(A.LIMIT_CD, 'CL001',  NVL(B.LIMIT_AMT, 0) + NVL(A.SAVE_AMT,0), 'CL003', A.AMT) AMT
FROM
(
  SELECT
